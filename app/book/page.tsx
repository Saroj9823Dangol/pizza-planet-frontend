...
import { createReservation, ReservationConfirmation } from "@/lib/api";
...
function BookPageInner() {
  ...
  const [location, setLocation] = useState(
    LOCATIONS.some((l) => l.slug === preselect) ? preselect! : LOCATIONS[0].slug,
  );
  ...
  useEffect(() => {
    if (preselect && LOCATIONS.some((l) => l.slug === preselect)) {
  ...
  const loc = useMemo(() => LOCATIONS.find((l) => l.slug === location), [location]);
          ...
          <form onSubmit={submit} className="mx-auto mt-12 max-w-[640px]">
            ...
            <div className="mt-10 text-center">
              ...
              <p className="mt-4 font-mono text-[0.55rem] tracking-[0.15em] text-ink-faint">
                {loc?.seats} SEATS · KITCHEN TAKES ORDERS UNTIL {loc?.hours.split("–")[1]?.trim()}
              </p>
            </div>
          </form>
        )}
