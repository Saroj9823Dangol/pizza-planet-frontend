...
const CRUSTS = [
  { id: "classic", label: "Classic", note: "soft, airy edge" },
  { id: "thin", label: "Thin", note: "crisp and light" },
  { id: "cheese", label: "Cheese edge", note: "extra mozzarella" },
];
...
export default function PizzaBuilder() {
  ...
  const [crust, setCrust] = useState(CRUSTS[0].id);
  ...
  const unitPrice = (selectedVariant?.price ?? selectedItem?.basePrice ?? 0) + toppingTotal;
  ...
  const addToTray = () => {
    ...
    addItem({
      id: `builder-${selectedItem.id}-${selectedVariant.id}-${selectedToppingIds.slice().sort().join("-")}-${crust}`,
